    ...
    db.transaction(() => {
        ...
        if (messages.length > 0) {
            for (let msg of messages) {
                let info = db.mutate(`INSERT OR IGNORE INTO messages_binary (timestamp, is_encrypted, content)
             VALUES (?, ?, ?)`, [
                    msg.getTimestamp(),
        ...
        db.mutate('INSERT INTO messages_merkles (id, merkle) VALUES (1, ?) ON CONFLICT (id) DO UPDATE SET merkle = ?', [JSON.stringify(trie), JSON.stringify(trie)]);
        returnValue = trie;
    });
    return returnValue;
}
...
export function sync(messages, since, groupId) {
    ...
    let newMessages = db.all(`SELECT * FROM messages_binary
         WHERE timestamp > ?
