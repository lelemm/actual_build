...
import { InvalidInputDataError, InvalidGoCardlessTokenError, AccessDeniedError, NotFoundError, ResourceSuspended, RateLimitError, UnknownError, ServiceError, RequisitionNotLinked, AccountNotLinedToRequisition, } from '../../errors.js';
import { mockedBalances, mockUnknownError, mockTransactions, mockDetailedAccount, mockInstitution, mockAccountMetaData, mockAccountDetails, mockRequisition, mockDeleteRequisition, mockCreateRequisition, mockRequisitionWithExampleAccounts, mockDetailedAccountExample1, mockDetailedAccountExample2, mockExtendAccountsAboutInstitutions, } from './fixtures.js';
...
describe('goCardlessService', () => {
    ...
    describe('#createRequisition', () => {
        ...
        it('handle error if status_code present in the response', async () => {
            ...
            await expect(() => goCardlessService.createRequisition(params)).rejects.toThrow(UnknownError);
        });
    });
    describe('#deleteRequisition', () => {
        ...
        it('handle error if status_code present in the response', async () => {
            ...
            await expect(() => goCardlessService.deleteRequisition(requisitionId)).rejects.toThrow(UnknownError);
        });
    });
    describe('#getRequisition', () => {
        ...
        it('handle error if status_code present in the response', async () => {
            ...
            await expect(() => goCardlessService.getRequisition(requisitionId)).rejects.toThrow(UnknownError);
        });
    });
    describe('#getDetailedAccount', () => {
        ...
        it('handle error if status_code present in the detailedAccount response', async () => {
            ...
            getMetadataSpy.mockResolvedValue(mockAccountMetaData);
            await expect(() => goCardlessService.getDetailedAccount(accountId)).rejects.toThrow(UnknownError);
            expect(getDetailsSpy).toBeCalledTimes(1);
            expect(getMetadataSpy).toBeCalledTimes(1);
        });
    ...
    describe('#getInstitutions', () => {
        ...
        it('handle error if status_code present in the response', async () => {
            ...
            await expect(() => goCardlessService.getInstitutions({ country })).rejects.toThrow(UnknownError);
        });
    });
    describe('#getInstitution', () => {
        ...
        it('handle error if status_code present in the response', async () => {
            ...
            await expect(() => goCardlessService.getInstitution(institutionId)).rejects.toThrow(UnknownError);
        });
    });
    ...
    describe('#getTransactions', () => {
      ...
      `);
        ...
        it('handle error if status_code present in the response', async () => {
            ...
            await expect(() => goCardlessService.getTransactions({
                institutionId: 'SANDBOXFINANCE_SFIN0000',
                accountId,
                startDate: '',
                endDate: '',
    ...
    describe('#getBalances', () => {
        ...
        it('handle error if status_code present in the response', async () => {
            ...
            await expect(() => goCardlessService.getBalances(accountId)).rejects.toThrow(UnknownError);
        });
    });
});
