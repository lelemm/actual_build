...
export async function bootstrap(loginSettings) {
    ...
    const passEnabled = Object.prototype.hasOwnProperty.call(loginSettings, 'password');
    const openIdEnabled = Object.prototype.hasOwnProperty.call(loginSettings, 'openid');
...
export async function disableOpenID(loginSettings, checkFileConfig = true, checkForOldPassword = false) {
    if (checkFileConfig && config.loginMethod) {
        return { error: 'unable-to-change-file-config-enabled' };
    }
    if (checkForOldPassword) {
        ...
        let confirmed = passwordHash && bcrypt.compareSync(loginSettings.password, passwordHash);
        if (!confirmed) {
...
export function login(password) {
    ...
    if (c === 0) {
        userId = uuid.v4();
        accountDb.mutate('INSERT INTO users (id, user_name, display_name, enabled, master) VALUES (?, ?, ?, 1, 1)', [userId, '', '']);
        accountDb.mutate('INSERT INTO user_roles (user_id, role_id) VALUES (?, ?)', [userId, '213733c1-5645-46ad-8784-a7b20b400f93']);
    }
    else {
        let { id: userIdFromDb } = accountDb.first('SELECT id FROM users WHERE user_name = ?', ['']);
        userId = userIdFromDb;
    }
    let expiration = TOKEN_EXPIRATION_NEVER;
    if (config.token_expiration != 'never' &&
...
export function getUserPermissions(userId) {
    let accountDb = getAccountDb();
    const permissions = accountDb.all(`SELECT roles.permissions FROM users
                              JOIN user_roles ON user_roles.user_id = users.id
                              JOIN roles ON roles.id = user_roles.role_id
                              WHERE users.id = ?`, [userId]) || [];
    const uniquePermissions = Array.from(new Set(permissions.flatMap((rolePermission) => rolePermission.permissions.split(',').map((perm) => perm.trim()))));
