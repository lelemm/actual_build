...
import { RequisitionNotLinked, AccountNotLinedToRequisition, GenericGoCardlessError, } from './errors.js';
...
app.post('/transactions', handleError(async (req, res) => {
    ...
    try {
        const { iban, balances, institutionId, startingBalance, transactions: { booked, pending, all }, } = await goCardlessService.getTransactionsWithBalance(requisitionId, accountId, startDate, endDate);
        res.send({
            status: 'ok',
            data: {
