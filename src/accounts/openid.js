...
export async function bootstrapOpenId(config) {
    ...
    accountDb.mutate("INSERT INTO auth (method, display_name, extra_data, active) VALUES ('openid', 'OpenID', ?, 1)", [JSON.stringify(config)]);
    return {};
}
...
export async function loginWithOpenIdSetup(body) {
    ...
    accountDb.mutate('INSERT INTO pending_openid_requests (state, code_verifier, return_url, expiry_time) VALUES (?, ?, ?, ?)', [state, code_verifier, body.return_url, expiry_time]);
...
export async function loginWithOpenIdFinalize(body) {
    ...
    let accountDb = getAccountDb();
    let config = accountDb.first("SELECT extra_data FROM auth WHERE method = 'openid' AND active = 1");
    if (!config) {
        return { error: 'openid-not-configured' };
    }
    config = JSON.parse(config['extra_data']);
    ...
    let { code_verifier, return_url } = accountDb.first('SELECT code_verifier, return_url FROM pending_openid_requests WHERE state = ? AND expiry_time > ?', [body.state, Date.now()]);
    try {
        ...
        let { c } = accountDb.first('SELECT count(*) as c FROM users WHERE user_name <> ?', ['']);
        let userId = null;
        if (c === 0) {
            userId = uuid.v4();
            accountDb.mutate('INSERT INTO users (id, user_name, display_name, enabled, master) VALUES (?, ?, ?, 1, 1)', [userId, userInfo.email, userInfo.name ?? userInfo.email]);
            accountDb.mutate('INSERT INTO user_roles (user_id, role_id) VALUES (?, ?)', [userId, '213733c1-5645-46ad-8784-a7b20b400f93']);
        }
        else {
            let { id: userIdFromDb, display_name: displayName } = accountDb.first('SELECT id, display_name FROM users WHERE user_name = ? and enabled = 1', [userInfo.email]) || {};
            if (userIdFromDb == null) {
                return {
                    error: 'openid-grant-failed: user does not have access to Actual Budget',
                };
            }
