...
app.post('/sync', async (req, res) => {
    ...
    let currentFiles = accountDb.all('SELECT group_id, encrypt_keyid, encrypt_meta, sync_version FROM files WHERE id = ?', [file_id]);
    ...
    if (currentFile.sync_version == null ||
...
app.post('/user-get-key', (req, res) => {
    ...
    let accountDb = getAccountDb();
    let { fileId } = req.body;
    let rows = accountDb.all('SELECT encrypt_salt, encrypt_keyid, encrypt_test FROM files WHERE id = ?', [fileId]);
    if (rows.length === 0) {
...
app.post('/user-create-key', (req, res) => {
    ...
    accountDb.mutate('UPDATE files SET encrypt_salt = ?, encrypt_keyid = ?, encrypt_test = ? WHERE id = ?', [keySalt, keyId, testContent, fileId]);
...
app.post('/upload-user-file', async (req, res) => {
    ...
    let keyId = encryptMeta && typeof encryptMeta === 'string'
        ? JSON.parse(encryptMeta).keyId
        : null;
    if (!fileId || typeof fileId !== 'string') {
        throw new Error('fileId is required');
    }
    let currentFiles = accountDb.all('SELECT group_id, encrypt_keyid, encrypt_meta FROM files WHERE id = ?', [fileId]);
    ...
    if (rows.length === 0) {
        // it's new
        groupId = uuid.v4();
        accountDb.mutate('INSERT INTO files (id, group_id, sync_version, name, encrypt_meta, owner) VALUES (?, ?, ?, ?, ?, ?)', [fileId, groupId, syncFormatVersion, name, encryptMeta, user.user_id]);
        ...
        accountDb.mutate('UPDATE files SET sync_version = ?, encrypt_meta = ?, name = ? WHERE id = ?', [syncFormatVersion, encryptMeta, name, fileId]);
        res.send(JSON.stringify({ status: 'ok', groupId }));
    }
});
app.get('/download-user-file', async (req, res) => {
    ...
    let rows = accountDb.all('SELECT id FROM files WHERE id = ? AND deleted = FALSE', [fileId]);
    if (rows.length === 0) {
        res.status(400).send('User or file not found');
        return;
    }
...
app.get('/list-user-files', (req, res) => {
    ...
    const canSeeAll = getUserPermissions(session.user_id).findIndex((permission) => permission === 'ADMINISTRATOR') > -1;
    let accountDb = getAccountDb();
    let rows = canSeeAll
        ? accountDb.all('SELECT * FROM files')
        : accountDb.all(`SELECT files.* 
          FROM files
          WHERE files.owner = ?
        UNION
         SELECT files.*
          ...
          JOIN user_access
            ...
            AND user_access.user_id = ?`, [session.user_id, session.user_id]);
    res.send(JSON.stringify({
        status: 'ok',
        data: rows.map((row) => ({
            deleted: row.deleted,
...
app.get('/get-user-file-info', (req, res) => {
    ...
    let accountDb = getAccountDb();
    let fileId = req.headers['x-actual-file-id'];
    let rows = accountDb.all('SELECT * FROM files WHERE id = ? AND deleted = FALSE', [fileId]);
    if (rows.length === 0) {
        res.send(JSON.stringify({ status: 'error' }));
        return;
    }
